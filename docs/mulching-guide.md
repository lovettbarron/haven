# Mulching Guide

## Overview

Mulch both beds before vacation to reduce evaporation, keep roots cool, and suppress weeds during absence. Applied on **Day -3** of the pre-departure countdown, followed immediately by deep watering to settle the material and start moisture retention.

A 5-8cm mulch layer can reduce soil moisture loss by 50-70%, extending the time between watering visits.

---

## Material Assignment

| Material | Beds | Why |
|----------|------|-----|
| **Straw** | A, B | Better moisture retention for active veggie crops. Easy to spread. Decomposes over the season and adds organic matter. |

Both beds are corten steel. Straw works well for both of them.

---

## Mulch Volume Calculations

| Bed | Dimensions (cm) | Area (m2) | Volume at 5cm (L) | Volume at 8cm (L) | Material |
|-----|-----------------|-----------|--------------------|--------------------|----------|
| A   | 230 x 120       | 2.76      | 138                | 221                | Straw    |
| B   | 150 x 60        | 0.90      | 45                 | 72                 | Straw    |
| **Total** |           | **3.66**  | **183**            | **293**            |          |

### Shopping List

| Material | Quantity | Coverage |
|----------|----------|----------|
| Straw bales | 2-3 bales (~100L loose each) | Beds A and B (183-293L needed) |

Buy straw from a garden center or farm supply. Standard compressed bales expand to roughly 100L of loose straw each. Three bales provides comfortable coverage at the thicker end of the range.

---

## Per-Bed Instructions

**Depth target for both beds:** 5-8cm. Use a stick marked at 5cm and 8cm to check depth as you go.

### Bed A -- "Family Bed" (the BIG bed on the grass)

- Spread straw evenly around all plants in all three zones
- Keep mulch **2-3cm away from plant stems** to prevent rot
- Keep mulch clear of the reservoir fill tube in the south zone so it remains accessible
- Tuck straw under the trellis frame at the south end but not against climbing stems
- In the north zone (raspberries), spread straw around canes and chives

### Bed B -- "His Bed" (the SMALLER bed near the house/terrace)

- Spread straw evenly around all plants
- Keep mulch away from strawberry crowns (2-3cm gap) -- strawberry crowns rot if buried
- Work around the sunken mint pot at the bed edge
- Keep mulch away from the base of sensory plants (lamb's ear, thyme)

---

## Child Activity

**Framing:** "We are tucking the plants in for their nap while we are away."

Mulch spreading is a sensory-rich activity -- different textures, earthy smells, and the satisfying visual of beds transforming from bare soil to cozy blankets. The child helps with both beds:

- **Bed B:** Child starts with his own bed -- grabs handfuls of straw and places them around his strawberries and flowers. Father guides depth and stem clearance. The smaller bed finishes quickly, good for building confidence.
- **Bed A:** The big bed -- child works the south zone near the trellis while father handles the raspberry zone and middle zone. Good teamwork activity.

Let the child set the pace. If interest fades, father finishes the remaining area while child does something else.

---

## Post-Mulching

Immediately after mulching both beds:

1. **Deep water every bed** -- soak through the mulch layer until water pools briefly on the surface
2. For Bed A south zone: also fill the reservoir via the fill tube until water flows from the overflow
3. The deep watering settles the mulch, starts moisture retention, and saturates the soil column beneath

This post-mulch watering is the most important watering of the entire pre-departure sequence.
